...
import com.utils.开发常用.线程池.ExecutorConfig;
...
//    }
    ...
    public void taskThread() {
        /**多线程调用*/
        ExecutorService threadPoolTaskExecutor = ExecutorConfig.newFixedThreadPool( 15 );
        /*闭锁*/
        ...
        for (int i = 0 ; i < 100 ; i++) {
            ...
            threadPoolTaskExecutor.execute( ()-> {
                try {
                    Thread.sleep( 1000 );
                    System.out.println( Thread.currentThread().getName() + "执行成功!!" + finalI );
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
...
//        threadPoolTaskExecutor.shutdown();
        try {
            //阻塞主线程用
            latch.await();
//            threadPoolTaskExecutor.awaitTermination( 1,TimeUnit.MINUTES );
        }
        catch (Exception e) {
            e.printStackTrace();
        }
